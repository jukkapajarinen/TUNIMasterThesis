...
\section{Verkon rakentaminen} \label{ch:10_verkon_rakentaminen}
  ...
  \begin{equation}\alpha(v_A) = p^{-1}(v_A) = 8^{-1} = \frac{1}{8} = 0.125\end{equation}
...
\section{Verkon karsiminen} \label{ch:10_verkon_karsiminen}
  ...
  \begin{enumerate}
    ...
    \begin{equation}d_G(v) < max\{d_G(x) | x \in V(G)\} \land \alpha(v) > \frac{1}{|V(G)|} \cdot \sum\limits_{v \in V(G)} \alpha(v)\end{equation}
  \end{enumerate}
...
\section{Dijkstran algoritmin hyödyntäminen} \label{ch:10_dijkstran_algoritmin_hyodyntaminen}
  ...
  \begin{equation}d_G^\alpha(v_1, v_2) = min\{\alpha(P) | P:v_1 \rightarrow v_2 | v_1, v_2 \in V(G)\}\end{equation}
