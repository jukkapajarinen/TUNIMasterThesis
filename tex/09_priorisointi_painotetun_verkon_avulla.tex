...
\section{Perusmerkinnät ja käsitteet} \label{ch:09_perusmerkinnat_ja_kasitteet}
  ...
  \begin{itemize}
    ...
    \item \textbf{Verkko} \(G = V \cup E\) on joukko joka sisältää solmujoukon \(V\) ja kaarijoukon \(E\).
...
\section{Painofunktiot priorisointiin} \label{ch:10_painofunktiot_priorisointiin}
  ...
  Painofunktio yksittäiselle solmut \(v_a\) ja \(v_b\) yhdistävälle kaarelle \(e_{xy}\), eli siirtymälle saadaan myös käänteislukuna.
...
\section{Dijkstran algoritmin hyödyntäminen} \label{ch:10_dijkstran_algoritmin_hyodyntaminen}
  ...
  \begin{equation}d_G^\alpha(v_1, v_2) = \text{min}\{\alpha(P) | P:v_1 \rightarrow v_2 | v_1, v_2 \in V(G)\}\end{equation}
