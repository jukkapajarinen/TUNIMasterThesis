...
\section{Perusmerkinnät ja käsitteet} \label{ch:09_perusmerkinnat_ja_kasitteet}
  ...
  \begin{itemize}
    \item \(V := \{v_1, v_2, v_3\}\) \textbf{Solmujoukko} joka sisältää \emph{solmut} \(v_1\), \(v_2\) ja \(v_3\).
    \item \(E := \{e_1, e_2, e_3\}\) \textbf{Kaarijoukko} joka sisältää \emph{kaaret} \(e_1\), \(e_2\) ja \(e_3\).
    \item \(d_G(v_1)\) \textbf{Solmun asteluku}, eli \emph{solmuun} \(v_1\) liittyvien \emph{kaarten} lukumäärä.
    \item \(G_2 \subset G_1\) \textbf{Aliverkko}, eli \emph{verkko} \(G_2\) joka koostuu osasta \emph{verkon} \(G_1\) \emph{solmuja} ja \emph{kaaria}.
    \item \(v_1 \neq v_2, v_1 \rightarrow v_2\) \textbf{Verkon yhtenäisyys}, eli jokaiselle solmuparille \(v_1 \neq v_2\) on olemassa niitä yhdistävä \emph{kaari}.
    \item \(P = \{v_0, v_1, ..., v_n\}, v_0 \rightarrow v_n\) \textbf{Polku}, eli \emph{suunnattu solmujono} jota pitkin voidaan kulkea \emph{solmusta} \(v_0\) \emph{solmuun} \(v_n\).
    \item \(P = \{v_0, v_1, ..., v_n| e \in E_P, e \notin \{E_P \setminus \{e\} \}\}\) \textbf{Sykli}, eli \emph{polku}, jonka aloitus \(v_0\) ja lopetussolmu \(v_n\) on sama, mutta polun jokaista kaarta \(e\) kuljetaan vain kerran.
    \item \(d_G(v_1) = 0\) \textbf{Eristetty solmu}, eli \emph{solmu} jonka \emph{asteluku} on nolla.
    \item \(v_1 \rightarrow v_2, d_G(v_1) = 1 \lor d_G(v_2) = 1\) \textbf{Silta}, eli \emph{kaari} johon yhdistyvän \emph{solmun asteluku} on yksi ja jonka poistaminen epäyhteinäistää \emph{verkon}.
    \item \(v_x \rightarrow v_x\) \textbf{Silmukka}, eli \emph{kaari} jonka \emph{aloitussolmut} ja \emph{lopetussolmu} ovat sama \emph{solmu}.
    \item \(\alpha := V(G), E(G) \rightarrow \mathbb{N}\) \textbf{Painofunktion yleinen kuvaus} verkossa \(G\), solmuille \(V\) ja kaarille \(E\).
  \end{itemize}
...
\section{Painofunktiot priorisointiin} \label{ch:10_painofunktiot_priorisointiin}
  ...
  Painofunktio yksittäiselle solmut \(v_x\) ja \(v_y\) yhdistävälle kaarelle \(e_{xy}\), eli siirtymälle saadaan myös käänteislukuna.
...
\section{Verkon rakentaminen} \label{ch:10_verkon_rakentaminen}
  ...
  Painomatriisi, esimerkin \ref{tab:esimerkki_verkon_priorisointi_muuttujat} mukaiselle datalle ja siitä lasketuille painoarvoille on esitetty seuraavassa matriisissa \(M_G\).
